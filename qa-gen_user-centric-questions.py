"""
User-Centric QA Generator for Markdown Documentation

This script processes markdown documentation files and uses LangChain with an LLM 
to generate question-answer pairs that reflect how real users would ask questions.
The questions are generated from a user's perspective, combining content across
sections when appropriate, while ensuring answers remain accurate and comprehensive.
"""
...
# Local imports
from prompts import USER_CENTRIC_QA_GENERATION_PROMPT_V2
...
class UserCentricQAGenerator:
    """Generates user-centric QA pairs from markdown documentation."""
    ...
    def __init__(self, config_path: str = "config.yaml"):
        ...
        self.output_file = "user_centric_qa_results.csv"
    ...
    def _initialize_llm(self) -> BaseChatModel:
        ...
        if provider == "google-genai":
            ...
            return ChatGoogleGenerativeAI(
                model=model_id,
                temperature=model_params.get("test-temperature", 0.3),  # Slightly higher temperature for more creative questions
                max_output_tokens=model_params.get("max-tokens", 100000),
    ...
    def estimate_question_count(self, content: str) -> int:
        """Estimate the appropriate number of questions based on content length and complexity."""
        # Base estimation on character count but with more emphasis on content density
        char_count = len(content)
        
        # Count section headings as a proxy for topic density
        heading_count = len(re.findall(r'^#{1,3}\s+.+$', content, re.MULTILINE))
        
        # Adjust based on both content length and heading density
        base_count = char_count // 250  # More chars per question (250 vs 200 in original)
        heading_adjustment = min(heading_count * 1.5, heading_count + 5)  # 1-2 questions per heading, max +5
        
        estimated_count = max(3, min(25, int(base_count + heading_adjustment)))
        return estimated_count
    
    def process_file(self, file_path: Path) -> List[Dict[str, str]]:
        """Process an entire markdown file and generate user-centric QA pairs."""
        ...
        try:
            ...
            # Estimate question count based on content
            num_questions = self.estimate_question_count(content)
            logger.info(f"Estimated {num_questions} questions for {file_path}")
            
            # Generate QA pairs using the user-centric prompt
            response = self.llm.invoke(
                USER_CENTRIC_QA_GENERATION_PROMPT_V2.format(
                    content=content
                )
            )
    ...
    def save_to_csv(self, qa_pairs: List[Dict[str, str]], output_file: str = None) -> None:
        ...
        field_names = ["question", "answer", "source", "topic"]
    ...
    def run(self) -> None:
        ...
        md_files = list(self.docs_dir.glob("**/*.md"))  # Recursive search to include subdirectories
        ...
        logger.info("User-centric QA generation complete!")

def main():
    """Main entry point."""
    generator = UserCentricQAGenerator()
    generator.run()
